...
def visualize_added_pose_features(filepath, row, focus_heights, focus_angles, focus_landmarks):
    fig, axs = plt.subplots(2, 2, figsize=(20, 20))
    colors = ['red', 'blue', 'green', 'purple', 'orange','magenta']
    color_index = 0 
    
    open_image_file = Image.open(filepath)
    
    axs[0,0].imshow(open_image_file)
    axs[0,0].set_title(f"{row['image_filename']}, {row['theta']}")
    axs[0,0].axis('off')
 
    axs[0,1].set_title("Distance of Non Adjacent Landmarks")   
    for label, (height, P1x, P1y, P2x, P2y) in focus_heights.items():
        axs[0,1].plot([P1x, P2x], [P1y, P2y], color=colors[color_index], linewidth=2)
        axs[0,1].text(P1x + 0.1, P1y, f"{label}: {height:.2f}")
        axs[0,1].scatter(P1x, P1y, color=colors[color_index])
        axs[0,1].scatter(P2x, P2y, color=colors[color_index])
        color_index = (color_index + 1) % len(colors)
        
    #axs[0,1].set_xlim(0, 1)
    #axs[0,1].set_ylim(0, 1)
    #axs[0,1].set_xticks(np.arange(0, 1.05, 0.1))
    #axs[0,1].set_yticks(np.arange(0, 1.05, 0.1))
    #axs[0,1].grid(True)
    ...
    for label, (angle, Ax, Ay, Bx, By, Cx, Cy) in focus_angles.items():
        ...
        axs[1,0].add_patch(FancyArrowPatch((Bx, By), (Ax, Ay), arrowstyle='->', color=colors[color_index], mutation_scale=10))
        axs[1,0].add_patch(FancyArrowPatch((Bx, By), (Cx, Cy), arrowstyle='->', color=colors[color_index], mutation_scale=10))
        axs[1,0].text(Bx + 0.1, By, f"{label}: {angle:.2f}°")
        axs[1,0].scatter(Bx, By, color=colors[color_index])
        ...
        axs[1,0].add_patch(angle_arc)
        color_index = (color_index + 1) % len(colors)
    #axs[1,0].set_xlim(0, 1)
    #axs[1,0].set_ylim(0, 1)
    #axs[1,0].set_xticks(np.arange(0, 1.05, 0.1))
    #axs[1,0].set_yticks(np.arange(0, 1.05, 0.1))
    #axs[1,0].grid(True)

    ## Scatter Plot
    
    axs[1,1].set_title("Landmark Scatter Visualization")

    for label, (Lx, Ly) in focus_landmarks.items():
        axs[1,1].scatter(Lx, Ly, color=colors[color_index])
        axs[1,1].text(Lx + 0.1, Ly, f"{label}")
        color_index = (color_index + 1) % len(colors)

    axs[1,1].set_xlim(-1, 1)
    axs[1,1].set_ylim(-1, 1)
    axs[1,1].set_xticks(np.arange(-1, 1.05, 0.1))
    axs[1,1].set_yticks(np.arange(-1, 1.05, 0.1))
    axs[1,1].grid(True)
